...
# feature level
dxr.human <- as.data.frame(dxr.human[,c("featureID", "groupID", "pvalue")])
```
...
# feature level
dxr.sequin <- as.data.frame(dxr.sequin[,c("featureID", "groupID", "pvalue")])
```
...
pvals.human <- rowData(sumExp.human)[["fitDTUResult_BvsA"]]$regular_FDR
...
# Compute gene level q-values
pvals.sequin <- rowData(sumExp.sequin)[["fitDTUResult_BvsA"]]$regular_FDR
...
DTU.transcripts <- list(
  ...
  DEXSeq = dxr.sequin$featureID[dxr.sequin$pvalue < 0.05],

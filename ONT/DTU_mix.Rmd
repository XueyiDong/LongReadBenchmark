...
humanInc <- 
  ...
  geom_line(linetype=rep(c("solid", "solid", "solid", "longdash", "dashed"), rep(4, 5))) +
  geom_point(size=3, shape=rep(c(1, 2, 3, 4, 5), rep(4, 5))) +
  theme_bw() +
  scale_colour_manual(values = c("#ECD98B", "#AAAAC2",  "#03875C", "#9A4C43", "#4E3227"))+
  theme(text = element_text(size = 20))
...
pdf(file.path(OUT, "sequinGeneFDR.pdf"), height = 5, width = 8)
...
pdf(file.path(OUT, "sequinTranscriptFDR.pdf"), height = 5, width = 8)
sequinFDR <- 
...
pdf(file.path(OUT, "sequinGeneTPR.pdf"), height = 5, width = 8)
...
pdf(file.path(OUT, "sequinTranscriptTPR.pdf"), height = 5, width = 8)
sequinTPR <-
...
pdf(file.path(OUT, "analysisMixTx.pdf"), height = 8, width = 16)
plot_grid(

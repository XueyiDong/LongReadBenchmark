...
### human
```{r DRIMSeq, echo=TRUE, eval=FALSE}
samples <- data.frame(
  sample_id = samples,
...
ex.human <- lapply(x.human, function(x){
  ...
  diffSplice(efit.human, geneid = x$genes$gene_id, exonid=x$genes$feature_id)
})
...
ex.sequin <- lapply(x.sequin, function(x){
  ...
  diffSplice(efit.sequin, geneid = x$genes$gene_id, exonid=x$genes$feature_id)
})

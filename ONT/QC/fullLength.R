...
tx <- readRDS("tx.RDS")
tx <- as.character(tx)
txStat <- mclapply(tx[(1 + 9283 * (args - 1)) : min((9283 * args), length(tx))], function(x){
  ...
  cat(substring(x, 1,17),  "length:", readDF[sel[1], "tx_len"], "FL fraction:", fl95, "\n")
  # cnt = nrow(readDF.sel)
  # rm(readDF.sel)
...
# table(ind)
txStat <- matrix(unlist(txStat), nrow = 7)
...
saveRDS(txStat, paste0("full_length/txStat_final", args, ".RDS"))
# gc()
# cat("making plot.", "\n")
# Fig 3C
# library(viridis)

...
bambu_annot <- read.delim(paste0(annot_loc, "/ONT_extended_annotations_edited.gtf"), header = FALSE) 
flair_annot <- read.delim(paste0(annot_loc,"/flair.collapse.isoforms.gtf"), header = FALSE)
flames_annot <- read.delim(paste0(annot_loc,"/isoform_annotated.filtered_flames.gff3"), header = FALSE)
sqanti_annot <- read.delim(paste0(annot_loc,"/merged_ont_0.95.collapsed_classification.txt"))
stringtie_annot <- read.delim(paste0(annot_loc,"/ONT_stringtie_merged_edited.gtf"), header = FALSE)
talon_annot <- read.delim(paste0(annot_loc, "/ONT_filtered_annot_talon.gtf"), header = FALSE)
...
s_sqanti <- sqanti_annot[sqanti_annot$chrom=="chrIS",]
...
# bar plot
colnames(summary)[c(3,4)] <- c("known sequins", "false discoveries")
data <- pivot_longer(summary[,c(1,3,4)], c("known sequins", "false discoveries"), names_to = "category",values_to="count")
...
s_sqanti_sub <- sqanti_sub_annot[sqanti_sub_annot$V1=="chrIS" & sqanti_sub_annot$V3=="transcript",c(1:5,9)]
s_sqanti_sub_class <- sqanti_class_sub[sqanti_class_sub$chrom=="chrIS",]
...
s_sqanti_sub$count <- sub_sqanti_counts[match(str_replace(separate(s_sqanti_sub, V9, into=c("transcript","gene"), sep=";")$transcript, "transcript_id ",""), sub_sqanti_counts$Name),5]
...
s_sqanti_sub <- s_sqanti_sub_class[s_sqanti_sub$count >= 10,]
...
s_known_sqanti_sub <- unique(s_sqanti_sub_class[str_detect(s_sqanti_sub_class$associated_transcript, paste(c(s$NAME),collapse="|")),"associated_transcript"])
...
summary_sub <- data.frame(tool = c("bambu", "FLAIR", "FLAMES","SQANTI3", "StringTie2", "TALON"), 
                              ...
                              recovered_seq_40 = c(23,16,12,13,17,13))
summary_sub$fd <- summary_sub$seq_related_annot - (summary_sub$known_seq_120 + summary_sub$recovered_seq_40)

# bar plot
colnames(summary_sub)[c(3:5)] <- c("sequins kept", "sequins removed", "false discoveries")
data_sub <- pivot_longer(summary_sub[,c(1,3:5)], c("sequins kept", "sequins removed", "false discoveries"), names_to = "category",values_to="count")

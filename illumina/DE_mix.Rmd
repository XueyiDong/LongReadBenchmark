...
ggplot(anno, aes(x=logFC, y=logFC_edgeR))+
  geom_point() +
  geom_smooth(alpha=0.5, method='lm', formula= y~x) +
  annotate(geom="text", x = -4, y = 8, label = eq, parse = TRUE, size = 6) +
  # geom_abline(intercept = 0, slope = 1) +
...
ggplot(anno, aes(x=logFC, y=logFC_DESeq2))+
  geom_point() +
  geom_smooth(alpha=0.5, method='lm', formula= y~x) +
  annotate(geom="text", x = -4, y = 8, label = eq, parse = TRUE, size = 6) +
  # geom_abline(intercept = 0, slope = 1) +
...
ggplot(anno, aes(x=logFC, y=logFC_EBSeq))+
  geom_point() +
  geom_smooth(alpha=0.5, method='lm', formula= y~x)+
  annotate(geom="text", x = -4, y = 8, label = eq, parse = TRUE, size = 6) +
  # geom_abline(intercept = 0, slope = 1) +
...
ggplot(anno, aes(x=logFC, y=logFC_NOISeq))+
  geom_point() +
  geom_smooth(alpha=0.5, method='lm', formula= y~x) +
  annotate(geom="text", x = -4, y = 8, label = eq, parse = TRUE, size = 6) +
  # geom_abline(intercept = 0, slope = 1) +
...
ggplot(annoData, aes(x=exp, y=value, colour=sample)) +
  ...
  scale_colour_manual(values = c("#E59EA1", "#DED8A2", "#F7C83E", "#4D5C28", "#DA4472", "#B77E60"))+
  theme(text = element_text(size = 20)) +
  annotate(geom="text", x = 0.02, y = 12, label = eq, parse = TRUE, size = 6) 
